...
batch_size = 5 * 390  # 4 days * 390 minutes (assuming 1-minute intervals for intraday data)
batches = [Data_NOT_N[i:i + batch_size] for i in range(0, len(Data_NOT_N), batch_size)]
...
def training(genomes, config):
    ...
    current_batch = batches[batch_counter]
    batch_counter = (batch_counter + 1) % len(batches)
    
    rows_number = len(current_batch) - 1
    for i in range(10, rows_number):
        for j in range(len(traders)):
            if not traders[j].check_alive():
                continue
            decision = nets[j].activate(np.append(current_batch[i], traders[j].credit / 1000))

            # Apply order
            if decision[0] > 0.9:
                if traders[j].credit >= decision[1] * 1000:
                    traders[j].buy(decision[1] * 1000, current_batch[i+1][3])
                else:
                    ge[j].fitness -= 10  # Penalize for trying to buy without enough credit
            elif decision[0] < 0.1:
                if traders[j].holdings >= (decision[1] * 1000) / current_batch[i+1][3]:
                    traders[j].sell(decision[1] * 1000, current_batch[i+1][3])
                else:
                    ge[j].fitness -= 10  # Penalize for trying to sell without enough holdings

            # Apply fitness function
            total_capital = traders[j].credit + (traders[j].holdings * current_batch[i][3])
    ...
    best_trader = max(traders, key=lambda trader: trader.credit + (trader.holdings * current_batch[-1][3]))
    print(f"Best trader's total capital this generation: {best_trader.credit + (best_trader.holdings * current_batch[-1][3])}")
...
def evaluate_best_genome(best_genome, config):
    ...
    rows_number = len(Data_NOT_N) - 1
    for i in range(10, rows_number):
        if not trader.check_alive():
            break
        decision = net.activate(np.append(Data_NOT_N[i], trader.credit / 1000))
